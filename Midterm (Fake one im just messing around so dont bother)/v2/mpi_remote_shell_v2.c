...
static void server_loop(int world_size) {
    ...
    while (active_clients > 0) {
        ...
        /* Logging + history (không lưu lệnh trống) */
        char ts[64];
        ...
        if (cmd_buf[0] != '\0' &&
            ...
            if (hist_count < HISTORY_SIZE) {
                ...
                /* ring buffer: dịch lịch sử lên 1 */
...
/* "Level 3" client: chạy script rồi benchmark, cuối cùng gửi exit */
static void client_level3(int rank) {
    ...
    /* đảm bảo đã gửi "exit" trong script; nếu muốn an toàn hơn,
       có thể gửi thêm exit ở đây, nhưng sẽ hơi thừa */
}
    ...
    if (size < 2) {
        if (rank == 0) {
            fprintf(stderr,
                    ...
                    "Example: mpirun -np 3 ./mpi_remote_shell_v2\n");
        }
        MPI_Finalize();
        return EXIT_FAILURE;
    }
